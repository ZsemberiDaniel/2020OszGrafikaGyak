...
bool CMyApp::Init()
{
	...
	*/
	...
	m_gpuParticleVAO.Init({
		...
		{CreateAttribute<1, glm::vec3, sizeof(glm::vec3), sizeof(glm::vec3)>, m_gpuParticleBuffer}
	});
...
void CMyApp::Update()
{
	...
	glBufferSubData(GL_ARRAY_BUFFER, 0, sizeof(glm::vec3)*m_particleCount, &(m_particlePos[0][0]));
	glBindBuffer(GL_ARRAY_BUFFER, 0);

	glBindBuffer(GL_ARRAY_BUFFER, m_gpuParticleVelocityBuffer);
	glBufferSubData(GL_ARRAY_BUFFER, 0, sizeof(glm::vec3) * m_particleCount, &(m_particleVel[0][0]));
	glBindBuffer(GL_ARRAY_BUFFER, 0);
...
void CMyApp::GenerateParticlePos()
{
...
void CMyApp::GenerateParticleVel()
{
	...
	for (int i = 0; i < m_particleCount; ++i)
	{
		m_particleVel[i] = glm::vec3(2 * rnd(gen), 2 * rnd(gen), 2 * rnd(gen));
	}
}


void CMyApp::AddParticles(int n)
{
	m_particlePos.resize(m_particlePos.size() + n);
	m_particleVel.resize(m_particleVel.size() + n);
	...
	for (int i = m_particlePos.size() - 1; i >= m_particlePos.size() - n; i--)
	{
		m_particlePos[i] = glm::vec3(rnd(gen), rnd(gen), rnd(gen));
		m_particleVel[i] = glm::vec3(2 * rnd(gen), 2 * rnd(gen), 2 * rnd(gen));
	}
...
void CMyApp::Render()
{
	...
	m_linesProgram.Use();
	std::vector<glm::vec3> linePos(m_particleCount * 2);
	for (int i = 0; i < m_particleCount; i++)
	{
		linePos[2 * i] = m_particlePos[i];
		linePos[2 * i + 1] = m_particlePos[i] + 0.06f * m_particleVel[i];
	}

	m_linesProgram.SetUniform("mvp", m_camera.GetViewProj());
	m_linesProgram.SetUniform("points", linePos);
	m_linesProgram.SetUniform("color", glm::vec4(1, 0, 0, 1));
	glDrawArrays(GL_LINES, 0, (GLuint)linePos.size());
	m_linesProgram.Unuse();
	...
	{
		ImGui::SliderFloat("Time speed", &timeSpeed, 0.05f, 1.0f);
		ImGui::SliderFloat("Energy remaining", &energyRemaining, 0.05f, 1.2f);

		if (ImGui::Button("Generate pos"))
		{
			GenerateParticlePos();
		}

		if (ImGui::Button("Generate vel"))
		{
			GenerateParticleVel();
		}

		int previousParticleCount = m_particleCount;
		ImGui::SliderInt("Particle count", &m_particleCount, 10, 200);
		if (previousParticleCount < m_particleCount)
		{
			AddParticles(m_particleCount - previousParticleCount);
		}
	}
