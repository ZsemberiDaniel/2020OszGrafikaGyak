...
void CMyApp::InitPlane()
{
	...
	glGenVertexArrays(1, &m_plane_vaoID);
	glBindVertexArray(m_plane_vaoID);

	glGenBuffers(1, &m_plane_vboID);
	glBindBuffer(GL_ARRAY_BUFFER, m_plane_vboID);
	glBufferData(GL_ARRAY_BUFFER, vertices.size() * sizeof(Vertex), &vertices[0], GL_STATIC_DRAW);
	...
	glGenBuffers(1, &m_plane_ibID);
	glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, m_plane_ibID);
	glBufferData(GL_ELEMENT_ARRAY_BUFFER, sizeof(indices), indices, GL_STATIC_DRAW);
...
bool CMyApp::Init()
{
	...
	m_loc_eyePos = glGetUniformLocation(m_programID, "eye_pos");
...
void CMyApp::Render()
{
	...
	glm::mat4 world = glm::translate(glm::vec3(-2, 0, 0));
	...
	// - Gomb kirajzolása
	//
	world = glm::translate(glm::vec3(2, 0, 0));
