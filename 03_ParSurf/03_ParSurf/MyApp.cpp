...
bool CMyApp::Init()
{
	...
	Vertex circVert[C];
	for (int i = 0; i < C; i++) {
		float rad = 2.0f * M_PI * ((float)i / N);
		circVert[i].p = glm::vec3(glm::cos(rad), 0, glm::sin(rad));
		circVert[i].c = glm::normalize(glm::vec3(glm::cos(rad), 0, glm::sin(rad)));
	}
	...
	glBindBuffer(GL_ARRAY_BUFFER, circVBO); // tegyük "aktívvá" a létrehozott VBO-t
	// töltsük fel adatokkal az aktív VBO-t
	glBufferData(GL_ARRAY_BUFFER,	// az aktív VBO-ba töltsünk adatokat
		sizeof(circVert),		// ennyi bájt nagyságban
		circVert,	// errõl a rendszermemóriabeli címrõl olvasva
	...
	for (int i=0; i<N; ++i)
		for (int j=0; j<M; ++j)
		{
			...
			indices[6*i + j*3*2*(N) + 1] = (i)		+ (j)*	(N+1);
			indices[6*i + j*3*2*(N) + 0] = (i+1)	+ (j)*	(N+1);
			indices[6*i + j*3*2*(N) + 2] = (i)		+ (j+1)*(N+1);

			indices[6*i + j*3*2*(N) + 4] = (i+1)	+ (j)*	(N+1);
			indices[6*i + j*3*2*(N) + 3] = (i+1)	+ (j+1)*(N+1);
...
void CMyApp::DrawCylinder(glm::mat4 transform) 
{
	glm::mat4 mvp = m_matProj * m_matView * transform;
	...
	// teteje kirajzolasa
	glm::mat4 world = glm::translate(glm::vec3(0, 1.0f, 0)) * glm::rotate<float>(M_PI, glm::vec3(1, 0, 0));
	mvp = m_matProj * m_matView * (transform * world);
	glUniformMatrix4fv(m_loc_mvp, 1, GL_FALSE, &(mvp[0][0]));
	...
	// alja kirajzolasa
	mvp = m_matProj * m_matView * transform;
	glUniformMatrix4fv(m_loc_mvp, 1, GL_FALSE, &(mvp[0][0]));
...
void CMyApp::Render()
{
	...
	DrawCylinder(glm::rotate(time, glm::vec3(0, 1, 0)) * glm::rotate(time, glm::vec3(1, 0, 1)));
