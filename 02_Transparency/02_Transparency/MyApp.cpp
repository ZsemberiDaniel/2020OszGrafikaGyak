...
bool CMyApp::Init()
{
	...
	*/
	...
	wallPositions.resize(15);
	for (int i = 0; i < wallPositions.size(); i++) 
	{
		wallPositions[i] = glm::vec3(3 * i, 0, 0);
	}
...
void CMyApp::Render()
{
	...
	{
		float distance = glm::length(m_camera.GetAt() - wallPositions[i]);
		sorted[distance] = wallPositions[i];
	}
	...
	{
		glm::mat4 wallWorld = glm::translate(it->second);
